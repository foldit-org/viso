...
use super::{AnimationPlayer, StructureAnimator};
...
pub(crate) struct AnimationState {
    ...
    /// In-flight structural-delta animation, when a publish is mid-play.
    /// `Some` between the start of a sequence and its final step; dropped on
    /// completion. Driven by the engine's single `update` call site; reset
    /// for free with this struct on scene reset.
    pub(crate) player: Option<AnimationPlayer>,
}
